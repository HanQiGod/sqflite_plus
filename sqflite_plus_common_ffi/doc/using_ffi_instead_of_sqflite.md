...
Currently (as of 2020/07/01) sqflite only supports iOS/Android/MacOS. `sqflite_plus_common_ffi` allows supporting Windows and Linux
on DartVM or flutter.

* [sqflite_plus_common](https://pub.dev/packages/sqflite_plus_common) provides an abstracted [`DatabaseFactory`](https://pub.dev/documentation/sqflite_plus_common/latest/sqlite_api/DatabaseFactory-class.html) that allows another level
  of abstraction (for any target, not only flutter) above the plugin mechanism which is only for flutter.
* [sqflite_plus_common_ffi](https://pub.dev/packages/sqflite_plus_common_ffi) defines a global `databaseFactoryFfi` allowing supporting Linux and Windows on Flutter and on DartVM.
...
Ideally, packages requiring sqlite feature should only require a [`DatabaseFactory`](https://pub.dev/documentation/sqflite_plus_common/latest/sqlite_api/DatabaseFactory-class.html) parameter to allow using any implementation.
...
```dart
import 'package:sqflite_plus_common_ffi/sqflite_ffi.dart';
