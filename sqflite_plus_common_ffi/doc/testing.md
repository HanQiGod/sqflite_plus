## Unit testing `sqflite` code using `sqflite_plus_common_ffi`
...
An alternative solution is to use `sqflite_plus_common_ffi` since it allows running SQLite code on 
the desktop (MacOS/Windows/Linux).

### Setup

Include `sqflite_plus_common_ffi` as a `dev_dependency` in `pubspec.yaml`:
...
In order to use your existing `sqflite` code in flutter test, you have to [setup `sqflite_plus_common_ffi` to replace the global default flutter database factory](../doc/using_ffi_instead_of_sqflite.md).
...
Future main() async {
  // Setup sqflite_plus_common_ffi for flutter test
  setUpAll(() {
    // Initialize FFI
    sqfliteFfiInit();
...
```dart
@TestOn('vm')
library sqflite_plus_common_ffi.test.sqflite_ffi_doc_test;
