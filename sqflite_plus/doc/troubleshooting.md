...
- Check the GeneratedPluginRegistrant file that flutter run should have generated in your project contains
  ...
  Android:
  ```java
  SqflitePlugin.registerWith(registry.registrarFor("com.tekartik.sqflite_plus.SqflitePlugin"));
  ```
  iOS:
  ```objective-c
...
### Could not resolve all files for configuration ':sqflite_plus_android:androidJdkImage'
...
You could get a more recent version using [`sqflite_plus_common_ffi`](https://pub.dev/packages/sqflite_plus_common_ffi).
